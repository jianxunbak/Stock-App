...
import { ChevronDown, ChevronUp, Check, X, AlertCircle, Sparkles, Loader2 } from 'lucide-react';
...
const StockHealthCard = ({
    ...
    if (view === 'summary') {
        ...
        const grading = score >= 80 ? 'Excellent' : score >= 60 ? 'Good' : 'Needs Attention';
                    ...
                    <div className="health-checklist">
                        {items.map((item, index) => {
                            const statusConfig = {
                                ...
                                pending: { icon: Sparkles, color: 'var(--neu-text-tertiary)' },
                                evaluating: { icon: Loader2, color: 'var(--neu-color-favorite)', spin: true }
                            }[item.status || 'warn'];
                            const Icon = statusConfig.icon;

                            return (
                                <ButtonAnimator key={index} type="expansionItem" as="div" className="checklist-item">
                                    <div className={`status-icon status-${item.status} ${statusConfig.spin ? 'spin' : ''}`}>
