...
function useFabricPathHorizontal(width, height, distortion) {
    return useTransform([width, height, distortion], ([w, h, d]) => {
        ...
        const extraWidth = Math.max(0, sw - h);
        // Linear dampener is fine for this case
        const dampener = Math.pow(Math.min(1, extraWidth / 60), 3);
...
const FabricBackground = ({ active, isStrokeOnly = false }) => {
    ...
    const lastActive = useRef(null); // Changed to null to trigger on mount
    ...
    useEffect(() => {
        ...
        if (active) {
            ...
            // Only animate collapse if it was previously active
            runAnimation(distortion, [0, -6, 0], { duration, times: [0, 0.4, 1], ease: "easeInOut" });
        }
    ...
    if (type === 'fabricSearchBar') {
        ...
        return (
            <BaseComponent
                ...
                animate={{
                    scaleX: active ? 1.02 : 1,
                    scaleY: active ? 0.99 : 1,
                    filter: active ? "blur(0px) brightness(1.02)" : "blur(0px) brightness(1)",
                    ...props.animate
                }}
                {...props}
            >
