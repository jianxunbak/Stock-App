...
const TestPortfolioPage = () => {
    ...
    const [mainPortfolioItems, setMainPortfolioItems] = useState([]);
    const [mainPortfolioLoading, setMainPortfolioLoading] = useState(false);
    const [selectedCopyItems, setSelectedCopyItems] = useState([]);

    const handleClearAll = async () => {
        if (!portfolio || portfolio.length === 0) return;
        if (window.confirm("Are you sure you want to clear all holdings from this portfolio? This cannot be undone.")) {
    ...
    const handleCopyClick = async () => {
        ...
        setMainPortfolioLoading(true);
        const items = await fetchMainPortfolio();
        setMainPortfolioItems(items);
        setSelectedCopyItems([]); // Reset selection
        setMainPortfolioLoading(false);
    };

    const handleCopySubmit = async () => {
        const itemsToCopy = mainPortfolioItems.filter(item => selectedCopyItems.includes(item.id));
        await copyItemsFromMain(itemsToCopy);
    ...
    const handleRenamePortfolioSubmit = async (newName) => {
        ...
        const currentName = portfolioList.find(p => p.id === idToRename)?.name;
        ...
        if (portfolioList.find(p => p.name === trimmedName && p.id !== idToRename)) {
            alert("A portfolio with this name already exists.");
            return;
        }

        try {
            await renamePortfolio(idToRename, trimmedName);
            setIsRenaming(false);
            setRenamingId(null);
        } catch (error) {
    ...
    const handleDeletePortfolio = async (id) => {
        ...
        if (!window.confirm(`Are you sure you want to delete the portfolio "${portfolioName || id}"?`)) return;
            ...
            <Modal isOpen={showCopyModal} onClose={() => setShowCopyModal(false)}
                ...
                message={
                    <div className={styles.copyModalContainer} style={{ paddingRight: '0.5rem' }}>
                        ...
                        {mainPortfolioLoading ? (
                            <div className={styles.spinner} style={{ margin: '2rem auto' }} />
                        ) : mainPortfolioItems.length === 0 ? (
                            <p>No items found in Main Portfolio.</p>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.5rem', paddingLeft: '1rem', paddingBottom: '0.2rem', paddingTop: '0.2rem' }}>
                                    <button className="icon-btn" onClick={() => setSelectedCopyItems(mainPortfolioItems.map(i => i.id))} title="Select All">
                                        <CopyCheck size={16} />
                                    </button>
                                ...
                                {mainPortfolioItems.map(item => (
                                    <label key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.8rem', background: 'rgba(255,255,255,0.05)', borderRadius: '8px', cursor: 'pointer', border: '1px solid transparent', transition: 'all 0.2s' }}
